...
# Metadata Bucket
###############################################################################################################
resource "aws_s3_bucket" "eclipse_metadata_bucket" {
  count  = var.dr_parameters["deploy_s3"] ? 1 : 0
  bucket = "${var.name_prefix}-${var.env}-${var.region}-${var.product_name}-metadata-bucket"
  tags   = merge(tomap({ "access-env" = var.env }), var.tags)
}

# Versioning
resource "aws_s3_bucket_versioning" "versioning_eclipse_metadata_bucket" {
  count  = var.dr_parameters["deploy_s3"] ? 1 : 0
  bucket = aws_s3_bucket.eclipse_metadata_bucket[0].id
  versioning_configuration {
    status = "Enabled"
  }
}
...
# Life cycle configuration
resource "aws_s3_bucket_lifecycle_configuration" "eclipse_metadata_bucket_lifecycle_config" {
  count  = var.dr_parameters["deploy_s3"] ? 1 : 0
  bucket = aws_s3_bucket.eclipse_metadata_bucket[0].id
  rule {
    id = "expire_object"
...
# SSE encryption
resource "aws_s3_bucket_server_side_encryption_configuration" "eclipse_metadata_bucket_access_logging" {
  count  = var.dr_parameters["deploy_s3"] ? 1 : 0
  bucket = aws_s3_bucket.eclipse_metadata_bucket[0].bucket
  rule {
    apply_server_side_encryption_by_default {
      sse_algorithm     = "aws:kms"
      kms_master_key_id = var.kms_master_key_arn
...
resource "aws_s3_bucket_replication_configuration" "s3_bucket_replication" {
  ...
  rule {
    ...
    destination {
      bucket  = "arn:aws:s3:::${var.name_prefix}-${var.env}-${var.dr_parameters["dr_region"]}-${var.product_name}-metadata-bucket"
      account = var.account_id
      access_control_translation {
        owner = "Destination"
      }
